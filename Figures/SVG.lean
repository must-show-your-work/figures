...
private def renderShape : Shape Pos2 → String
  | .point id pos style =>
...
private def shapeAnchor : Shape Pos2 → Pos2
  | .point _ pos _       => pos
  | .segment _ a b _     => ⟨(a.x + b.x) / 2, (a.y + b.y) / 2⟩
  | .line _ a b _        => ⟨(a.x + b.x) / 2, (a.y + b.y) / 2⟩
...
private def anchorOf (shapes : Array (Shape Pos2)) (target : Name) : Option Pos2 :=
  (shapes.find? (fun s => shapeId s == target)).map shapeAnchor

private def renderAnnotation (shapes : Array (Shape Pos2)) (a : Annotation) : Option String :=
  match a with
  | .label target text => do
      let p ← anchorOf shapes target
...
def render (s : Scene Pos2) (canvas : Canvas := {}) : String :=
  ...
  let shapeLines := styledShapes.toList.map renderShape
  let annLines := s.annotations.toList.filterMap (renderAnnotation styledShapes)
  let header :=
